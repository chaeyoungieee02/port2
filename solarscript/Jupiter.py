...
jupiter.modifiers.new(name="Subdivision", type='SUBSURF')
jupiter.modifiers["Subdivision"].levels = 2
jupiter.modifiers["Subdivision"].render_levels = 3

# Create material and assign texture
texture_path = r"C:\Users\yuend\Downloads\Universe Factory\2ND YEAR - 3RD SEM\M&A\solarsystem_textures\2k_jupiter.jpg"
...
# Add Jupiter-specific material properties (gas giant appearance)
if 'Metallic' in bsdf.inputs:
    bsdf.inputs['Metallic'].default_value = 0.0  # Non-metallic gas giant
if 'Roughness' in bsdf.inputs:
    bsdf.inputs['Roughness'].default_value = 0.8  # More diffuse surface
# Using IOR (Index of Refraction) for gas giant atmosphere
if 'IOR' in bsdf.inputs:
    bsdf.inputs['IOR'].default_value = 1.2  # Lower IOR for gas atmosphere

# Assign material
if jupiter.data.materials:
    jupiter.data.materials[0] = mat
else:
...
# Jupiter rotates very fast (about 10 hours per day) - fastest in solar system
# Multiple full rotations during the 5-second animation to show rapid rotation
jupiter.rotation_euler = (0, 0, 0)
jupiter.keyframe_insert(data_path="rotation_euler", frame=0)
jupiter.rotation_euler = (0, 0, math.pi * 8)  # 4 full rotations (8π radians)
jupiter.keyframe_insert(data_path="rotation_euler", frame=120)

# Linear interpolation
if jupiter.animation_data and jupiter.animation_data.action:
...
print("Jupiter planet animation setup complete!")